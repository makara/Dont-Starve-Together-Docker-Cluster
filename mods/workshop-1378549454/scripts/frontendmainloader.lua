...
for i, modname in ipairs(ModManager:GetEnabledServerModNames()) do
    if modname == GEMENV.modname then break end --this is an ipairs list, which means that we just need to manually call frontend load mod for all the mods inbetween the start and gem cores
