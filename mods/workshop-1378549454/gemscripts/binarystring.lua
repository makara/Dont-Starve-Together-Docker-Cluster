...
local BinaryString = Class(function(self, binarypackedstring, debugoutput)
    ...
    function self:GetAsString()
        ...
        while writepos.position < size do
            vals[#vals + 1] = writepos:ReadByte()
            writepos = writepos + 1
            if #vals == 3 then
                strs[#strs + 1] = CreateStringFromValues(unpack(vals))
                vals = {}
            end
        end
        if #vals ~= 0 then
            strs[#strs + 1] = string.sub(CreateStringFromValues(vals[1], vals[2] or 0, 0), 1, (#vals == 1 and 2) or (3))..((#vals == 1 and "==") or ("="))
        end
    ...
    local function ValueWriteBytes(count, value, pos, updateposition)
        if type(value) == "number" then
            ...
            local _value = {}
            for i = count, 1, -1 do
                _value[#_value + 1] = bit.band(0xFF, value)
