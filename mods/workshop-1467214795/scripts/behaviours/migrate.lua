...
function Migrate:Visit()
    ...
    if distanceToHome and distanceToHome < MIGRATION_HOME_RADIUS_SQ then
        ...
        self.inst.components.locomotor:RemoveExternalSpeedMultiplier(self.inst, "migration")
        self.status = FAILED
    end
    ...
    if self.status == READY then
        ...
        self.status = RUNNING
        self.inst.components.locomotor:AddExternalSpeedMultiplier(self.inst, "migration", -0.8)
